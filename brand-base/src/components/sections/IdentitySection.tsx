...
export default function IdentitySection({ defaultOpen = false, lazyLoad = true }: Props) {
  ...
  return (
    <div ref={ref} className="container-responsive py-8 md:py-12">
      <Accordion title="Identity" isOpen={open} onOpenChange={setOpen} defaultOpen={defaultOpen}>
        {shouldLoad ? (
          <div className="opacity-0 animate-fade-in [animation-fill-mode:forwards]">
            <p className="text-sm opacity-80">Logos, color palettes, and typography specs.</p>
          </div>
        ) : null}
      </Accordion>
    </div>
