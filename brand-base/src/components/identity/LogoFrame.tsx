...
export default function LogoFrame({
  ...
  const renderLogo = () => {
    ...
    // Stacked - uses aspect ratio with side margins
    if (logoType === 'stacked') {
      return (
        <div className="relative shrink-0 size-[211.437px]">
          <div
              ...
              <div className="absolute top-full right-0 mt-2 flex flex-col gap-2 z-[30]">
                {colorVariants.map((color) => {
                  ...
                  const getColorButtonStyle = () => {
                    ...
                    // Unselected states - text color depends on background
                    const isVanillaBackground = currentColor === 'Vanilla'

                    if (color === 'Charcoal') {
                      // Charcoal button: charcoal fill with text based on container background
                      const textColor = isVanillaBackground ? 'text-brand-vanilla' : 'text-brand-charcoal'
